...
namespace BLiveSitePlugin
{
    static class Tools
    {
        ...
        public static async Task<string> GetLiveId(IDataSource dataSource, string input)
        {
            //LIVE_ID
            //CHANNEL_ID
            //https://www.blive.tv/live/CHANNEL_ID
            //https://www.blive.tv/live/LIVE_ID
            //https://www.blive.tv/user/CHANNEL_ID

            string id;
            var match = Regex.Match(input, "blive\\.tv/(?:live|movie|user)/(?<id>[^?/&]+)");
            if (match.Success)
            {
        ...
        {
            var b = Regex.IsMatch(input, "blive\\.tv/user/(?<programid>[^?/&=]+)");
            return b;
        }
        public static string ExtractChannelId(string input)
        {
            var match = Regex.Match(input, "blive\\.tv/user/([^?/&=]+)");
            if (match.Success)
            {
        ...
        {
            var b = Regex.IsMatch(input, "blive\\.tv/((?:live)|(?:movie))/(?<programid>[^?/]+)");
            return b;
        }
        ...
        {
            var ret = "";
            const string pattern = "blive\\.tv/((?:live)|(?:movie))/(?<programid>[^?/]+)";
